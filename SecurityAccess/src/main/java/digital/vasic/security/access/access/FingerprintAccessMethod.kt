...
import digital.vasic.security.access.ui.FingerprintAccessActivity
...
class FingerprintAccessMethod(
    ...
    override val accessMethod: AccessMethod = AccessMethod.FINGERPRINT
    ...
    override fun checkCapability(callback: digital.vasic.security.access.utils.CapabilityCheckCallback) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val fingerprintEnabled = settings?.allowFingerprint ?: true

                val capable = canAuthenticate && fingerprintEnabled
    ...
    override fun install() {
        // Fingerprint access doesn't require additional installation beyond hardware availability
    ...
    /**
     * Setup fingerprint access for the first time
     */
    suspend fun setupFingerprint(): SetupResult {
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (!canAuthenticate) {
                    return@withContext SetupResult.Error("Fingerprint authentication not available on this device")
                }
                ...
                if (!enrolled) {
                    return@withContext SetupResult.Error("No fingerprints enrolled. Please enroll fingerprints in device settings first.")
                }

                // Update settings to enable fingerprint access
                ...
                SetupResult.Error("Failed to setup fingerprint access: ${e.message}")
            }
        }
    }

    companion object {
        const val FINGERPRINT_ACCESS_REQUEST_CODE = 1003
    }
